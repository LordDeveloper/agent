...
ADS_HOSTS_PATH = Path("/var/lib/agent/ads-block-hosts")
STEVEN_BLACK_HOSTS_URL = "https://raw.githubusercontent.com/StevenBlack/hosts/master/hosts"
HOSTS_REFRESH_MAX_AGE_SECONDS = 7 * 24 * 3600
HOSTS_SKIP_NAMES = frozenset(
    {
        "localhost",
...
def parse_hosts_blocklist(text: str) -> list[str]:
    hosts: list[str] = []
    seen: set[str] = set()
    for line in text.splitlines():
        ...
        for host in parts[1:]:
            ...
            if not normalized or normalized in HOSTS_SKIP_NAMES or normalized in seen:
                continue
            seen.add(normalized)
...
def write_ads_hosts_file(hosts: list[str], path: Path | None = None) -> int:
    target = path or ADS_HOSTS_PATH
    target.parent.mkdir(parents=True, exist_ok=True)
    unique_hosts = sorted(set(hosts))
    lines = ["# Generated by Netinja agent — ads/tracker hosts blocklist"]
    lines.extend(f"0.0.0.0 {host}" for host in unique_hosts)
    target.write_text("\n".join(lines) + "\n", encoding="utf-8")
    return len(unique_hosts)


def refresh_ads_hosts_file(*, force: bool = False, runner: Runner | None = None) -> dict[str, Any]:
    """Download StevenBlack hosts list (weekly) for dnsmasq addn-hosts."""
    del runner
    now = time.time()
    if (
        not force
        and ADS_HOSTS_PATH.is_file()
        and (now - ADS_HOSTS_PATH.stat().st_mtime) < HOSTS_REFRESH_MAX_AGE_SECONDS
    ):
        try:
            existing = parse_hosts_blocklist(ADS_HOSTS_PATH.read_text(encoding="utf-8", errors="ignore"))
        except OSError:
            existing = []
        if existing:
            return {
    ...
    try:
        request = urllib.request.Request(
            STEVEN_BLACK_HOSTS_URL,
            headers={"User-Agent": "netinja-agent/ads-block"},
        )
        with urllib.request.urlopen(request, timeout=60) as response:
            payload = response.read().decode("utf-8", errors="ignore")
        hosts = parse_hosts_blocklist(payload)
        if hosts:
            downloaded = True
            source = "stevenblack"
    except (urllib.error.URLError, TimeoutError, OSError) as exc:
        log.warning("ads_block: failed to download hosts list: %s", exc)

    if not hosts and ADS_HOSTS_PATH.is_file():
        try:
            hosts = parse_hosts_blocklist(ADS_HOSTS_PATH.read_text(encoding="utf-8", errors="ignore"))
            source = "existing"
        except OSError:
            hosts = []

    if not hosts:
        hosts = list(DEFAULT_ADS_DOMAINS)
        source = "curated"

    count = write_ads_hosts_file(hosts)
    return {
        "ok": True,
        "refreshed": downloaded or force,
        "hosts": count,
        "path": str(ADS_HOSTS_PATH),
        "source": source,
    }
...
def render_dnsmasq_ads_conf(domains: list[str], *, hosts_file: Path | None = None) -> str:
    lines = [
        "# Generated by Netinja agent — ads blocklist for VPN DNS.",
        "# Clients that use this resolver (Interface DNS) get ads NX/blackholed.",
    ]
    target_hosts = hosts_file or ADS_HOSTS_PATH
    if target_hosts.is_file():
        lines.append(f"addn-hosts={target_hosts}")
...
def ads_block_status(*, runner: Runner | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "hosts_path": str(ADS_HOSTS_PATH),
        "hosts_entries": (
            len(parse_hosts_blocklist(ADS_HOSTS_PATH.read_text(encoding="utf-8", errors="ignore")))
            if ADS_HOSTS_PATH.is_file()
            else 0
        ),
...
def ads_block_ensure(*, runner: Runner | None = None) -> dict[str, Any]:
    ...
    hosts_meta = refresh_ads_hosts_file(runner=runner)
    ADS_DROPIN.parent.mkdir(parents=True, exist_ok=True)
    ADS_DROPIN.write_text(
        render_dnsmasq_ads_conf(domains, hosts_file=ADS_HOSTS_PATH),
        encoding="utf-8",
    )

...
DEFAULT_ASSET = "agent-linux-amd64"
...
def resolve_asset_name(explicit: str | None = None) -> str:
    return (explicit or os.environ.get("AGENT_GITHUB_ASSET") or DEFAULT_ASSET).strip()
...
def fetch_latest_release(
    *,
    ...
    if match is None:
        ...
        raise AgentError(
            "CONFIG_NOT_FOUND",
            f"Asset [{wanted}] missing from release [{tag}]. Available: {', '.join(names) or 'none'}",
        )
...
def _assert_linux_amd64() -> None:
    system = platform.system().lower()
    machine = platform.machine().lower()
    if system != "linux":
        raise AgentError("UNSUPPORTED_CAPABILITY", f"Self-update supports Linux only (got {system})")
    if machine not in {"x86_64", "amd64"}:
        raise AgentError(
            "UNSUPPORTED_CAPABILITY",
            f"Self-update currently ships amd64 only (got {machine})",
        )

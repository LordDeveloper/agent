...
def test_ensure_dnsmasq_systemd_unit_creates_agent_unit(tmp_path, monkeypatch):
    ...
    assert '--conf-dir=' in unit_path.read_text(encoding='utf-8')
...
def test_ensure_dnsmasq_service_disables_resolved_stub_on_port_conflict(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda runner=None: {'ok': True, 'unit': 'agent-dnsmasq', 'created': False, 'installed': False},
    )
    monkeypatch.setattr('agent.dns_leak.dnsmasq_systemd_unit', lambda runner=None: 'agent-dnsmasq')

    calls: list[list[str]] = []

    def fake_run(args, **kwargs):
        calls.append(list(args))
        if args[:3] == ['systemctl', 'is-active', 'agent-dnsmasq']:
            return _Result(stdout='inactive\n')
        if args[:3] == ['systemctl', 'is-enabled', 'agent-dnsmasq']:
...
def test_ensure_dnsmasq_service_does_not_touch_resolved_without_port_conflict(tmp_path, monkeypatch):
    ...
    def fake_run(args, **kwargs):
        ...
        if args[:3] == ['systemctl', 'status', 'agent-dnsmasq']:
...
def test_ensure_dnsmasq_service_starts_agent_unit(tmp_path, monkeypatch):
    dropin = tmp_path / 'resolved.conf.d' / 'netinja-dnsmasq.conf'
    unit_path = tmp_path / 'agent-dnsmasq.service'
    monkeypatch.setattr('agent.dns_leak.RESOLVED_STUB_DROPIN', dropin)
    monkeypatch.setattr('agent.dns_leak.AGENT_DNSMASQ_UNIT_PATH', unit_path)
    ...
    monkeypatch.setattr(
        'agent.dns_leak.ensure_dnsmasq_systemd_unit',
        lambda runner=None: {'ok': True, 'unit': 'agent-dnsmasq', 'created': True, 'installed': False},
    )
    ...
    monkeypatch.setattr('agent.dns_leak.dnsmasq_service_active', lambda runner=None: True)

    payload = ensure_dnsmasq_service(runner=fake_run)

    assert payload['active'] is True
    assert 'created_dnsmasq_unit' in payload['actions']
    assert any(args[:3] == ['systemctl', 'enable', 'agent-dnsmasq'] for args in calls)
    assert any(args[:3] == ['systemctl', 'restart', 'agent-dnsmasq'] for args in calls)
